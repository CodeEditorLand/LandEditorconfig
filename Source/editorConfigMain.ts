	...
	if (ctx.globalState.get("chrisdias.vscodeEditorConfig.informUser", true)) {
		window
			.showInformationMessage(
				"This version of EditorConfig has been deprecated, more information is available online.",
				"How to Upgrade",
			)
			.then((selection) => {
				if (selection) {
					open(
						"https://marketplace.visualstudio.com/items?itemName=chrisdias.vscodeEditorConfig",
					);
				}
			});
		// only prompt one time
		ctx.globalState.update(
			"chrisdias.vscodeEditorConfig.informUser",
			false,
		);
	}
...
`;
	...
	["indent_style", "indent_size", "tab_width"].forEach((setting) => {
